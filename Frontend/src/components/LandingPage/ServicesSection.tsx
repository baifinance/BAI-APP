    ...
    <section id="services" className="py-24 bg-slate-50/70 border-t border-slate-200/60 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-[#0038A8] text-xs font-extrabold uppercase tracking-widest mb-6">
            <span className="w-2 h-2 rounded-full bg-[#0038A8]" />
            <span>OUR SERVICES</span>
          </div>

          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0038A8] tracking-tight leading-tight mb-4">
            Services built for{" "}
        ...
        <div className="flex items-center justify-center flex-wrap gap-2.5 sm:gap-3 mb-12">
          <button
            ...
            className={`px-5 py-2.5 rounded-2xl text-xs sm:text-sm font-bold transition-all duration-200 ${
              activeTab === "all"
                ? "bg-[#0038A8] text-white shadow-md shadow-[#0038A8]/20 scale-[1.02]"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 min-h-[380px]">
          {displayedServices.map((service) => (
            <div
              key={service.id}
              className="group bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/90 hover:border-[#0038A8]/50 shadow-xs hover:shadow-soft-xl transition-all duration-300 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link
                    href="#book"
                    className="w-8 h-8 rounded-full bg-slate-50 group-hover:bg-slate-100 flex items-center justify-center text-slate-400 group-hover:text-[#0038A8] transition-colors"
                ...
                <h3 className="text-xl font-bold text-[#0038A8] mb-3 group-hover:text-[#002266] transition-colors">
                  {service.title}
                </h3>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <Link
                  href="#book"
                  className="text-xs font-bold text-[#0038A8] hover:underline flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  <span>Inquire</span>
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex items-center justify-center gap-4">
            <button
              ...
              className="w-9 h-9 rounded-full bg-white border border-slate-200 text-slate-600 hover:text-[#0038A8] hover:border-[#0038A8]/40 hover:bg-slate-50 flex items-center justify-center transition-all shadow-xs"
              ...
              {Array.from({ length: totalPages }).map((_, index) => (
                <button
                  ...
                  className={`transition-all duration-300 ${
                    currentPage === index
                      ? "w-8 h-2.5 bg-[#0038A8] rounded-full shadow-xs"
